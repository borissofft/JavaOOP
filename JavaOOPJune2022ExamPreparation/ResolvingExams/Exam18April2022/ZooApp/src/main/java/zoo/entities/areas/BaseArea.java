...
public abstract class BaseArea implements Area {
    ...
    public String getInfo() {
        ...
        if (this.animals.size() == 0) {
            ...
            sb.append(String.format("%s", this.animals.stream().map(Animal::getName).collect(Collectors.joining(" ")))).append(System.lineSeparator());
        }
        sb.append(String.format("Foods: %d", this.foods.size())).append(System.lineSeparator());
        int sumCalories = this.sumCalories();
